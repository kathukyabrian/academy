...
- can be done through *queryLookupStrategy* of the __EnableJpaRepositories__ annotation
- 2 ways
  - deriving query from method directly
  - using a manually derived query
...
- combines create and use_declared_query. looks up for a declared query first, if not found it creates a custom query
...
```
- parsing is divided into
  - subject
  - predicate
- first part find...By defines the subject, the second part the predicate
- any text between find(or any other introducing keywords) and by is considered to be descriptive unless using one of the result limiting keyword eg distinct, top, first
- general rules of trade:
  - expressions are usually property traversals combined with operators eg AND, OR, BETWEEN, LESSTHAN, GREATERTHAN, LIKE
  - parser supports setting an IgnoreCase flag for individual properties
  - you can apply static ordering by appending an *OrderBy* clause to the query method that references a property and providing the sorting direction